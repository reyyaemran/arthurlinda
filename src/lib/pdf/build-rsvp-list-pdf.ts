...
import type { Rsvp, Wedding } from "@/types/wedding";
...
export function buildRsvpListPdf(input: { wedding: Wedding; rsvps: Rsvp[] }): Uint8Array {
  const { wedding, rsvps } = input;
  ...
  // Compact columns requested by admin:
  // name, phone, email, status, pax, notes, submitted/updated date.
  const col = {
    name: 130,
    phone: 90,
    email: 165,
  ...
  for (const r of rsvps) {
    ...
    const dateText = r.submittedAt
      ? format(new Date(r.submittedAt), "MMM d, yyyy HH:mm")
      : "—";
    const cells: Record<(typeof colOrder)[number], string> = {
      name: trunc(r.name, 34),
      phone: trunc(r.phone || "—", 22),
      email: trunc(r.email || "—", 34),
      status: PDF_RSVP_STATUS[r.attendance],
      pax: r.paxCount > 0 ? String(r.paxCount) : "—",
      notes: trunc(r.message?.trim() || "—", 54),
  ...
  const foot = `${rsvps.length} response${rsvps.length === 1 ? "" : "s"} · exported ${format(new Date(), "yyyy-MM-dd HH:mm")}`;
